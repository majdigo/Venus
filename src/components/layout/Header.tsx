...
import { NAVIGATION_CATEGORIES, WHATSAPP_URL, PHONE_NUMBER_HREF, type NavCategory } from '@/lib/navigation-data';
...
export function Header() {
  ...
  return (
    <>
      <header
        className={`w-full bg-white/95 backdrop-blur-md sticky top-0 z-50 border-b border-border transition-shadow duration-300 ${
        ...
        <div className="container mx-auto px-4 h-20 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-6 text-sm font-semibold text-slate-700">
            ...
            <div className="relative group">
              <button className="flex items-center gap-1 hover:text-brand-gold transition-colors py-2">
              ...
              <div className="absolute top-full left-0 mt-1 bg-white rounded-xl shadow-xl border border-slate-100 py-2 w-52 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 translate-y-1 group-hover:translate-y-0">
                <Link href="/chirurgiens/" className="block px-4 py-2.5 text-sm text-slate-700 hover:text-brand-gold hover:bg-slate-50 transition-colors">
                  Nos Chirurgiens
                </Link>
                <Link href="/cliniques-partenaires/" className="block px-4 py-2.5 text-sm text-slate-700 hover:text-brand-gold hover:bg-slate-50 transition-colors">
                  Cliniques Partenaires
                </Link>
                <Link href="/sejour-medical/" className="block px-4 py-2.5 text-sm text-slate-700 hover:text-brand-gold hover:bg-slate-50 transition-colors">
                  Séjour Médical
                </Link>
              </div>
            </div>

            <Link href="/tarifs/" className="hover:text-brand-gold transition-colors py-2">
              Tarifs
            </Link>
            <Link href="/temoignages/" className="hover:text-brand-gold transition-colors py-2">
              Témoignages
            </Link>
            <Link href="/contact/" className="hover:text-brand-gold transition-colors py-2">
              Contact
            </Link>
          </nav>
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            <Link href="/devis/">
              <Button className="bg-brand-gold text-white hover:bg-brand-gold/85 font-bold shadow-md">
                Devis Gratuit
              </Button>
            </Link>
          </div>
              ...
              <SheetContent side="left" className="w-full sm:w-[380px] p-0 flex flex-col">
                  ...
                  {mobileSubmenu === null ? (
                    ...
                    <div className="py-4">
                      <button
                        ...
                        className="flex items-center gap-2 px-6 py-3 text-sm font-medium text-brand-gold hover:text-brand-gold/85"
                      >
                            ...
                            {cat.interventions.map((int) => (
                              <li key={int.slug}>
                                <Link
                                  ...
                                  className="flex items-center justify-between py-1.5 text-sm text-slate-700 hover:text-brand-gold transition-colors"
                                >
                ...
                <div className="border-t border-slate-200 p-4">
                  <Link href="/devis/" onClick={() => { setMobileOpen(false); setMobileSubmenu(null); }}>
                    <Button className="w-full bg-brand-gold text-white hover:bg-brand-gold/85 font-bold h-12 text-base">
            ...
            <div className="container mx-auto px-4 py-8 max-w-7xl">
              ...
              <div className="mt-6 pt-6 border-t border-slate-100">
                <Link
                  ...
                  className="flex items-center justify-center gap-2 bg-brand-gold/10 hover:bg-brand-gold/20 text-brand-gold/85 font-bold py-3 px-6 rounded-xl transition-colors"
                >
        ...
        {category.interventions.map((int) => (
          <li key={int.slug}>
            <Link
              ...
              onClick={onClose}
              className="flex items-center justify-between text-sm text-slate-600 hover:text-brand-gold hover:translate-x-1 transition-all py-1"
            >
              <span className="flex items-center gap-2">
                {int.name}
                {int.isPopular && (
                  <Badge variant="secondary" className="text-[10px] px-1.5 py-0 bg-brand-gold/10 text-brand-gold/85 border-0">
                    Populaire
                  </Badge>
                )}
              </span>
